...
export async function runDebate(topic, turns = DEFAULT_TURNS, onEvent) {
  ...
  // Phase 1: Opening statements (parallel with individual error handling)
...

{
  ...
  "action": "insist"
    ...
    try {
      ...
      console.error(`[DEBATE] Opening failed for ${char.name}:`, err.message);
      return {
        turn: 0,
        characterId: char.id,
        name: char.name,
        emoji: char.emoji,
        color: char.color,
        speech: `[${char.name}의 마이크에 기술적 문제가 발생했습니다: ${err.message}]`,
        action: 'insist',
        target: null,
      };
    }
...

{
  ...
  "target": "지지할 경우 대상 이름, 고수할 경우 null"
      ...
      try {
        ...
        console.error(`[DEBATE] Turn ${turn} failed for ${char.name}:`, err.message);
        return {
          turn,
          characterId: char.id,
          name: char.name,
          emoji: char.emoji,
          color: char.color,
          speech: `[통신 장애로 ${char.name}이(가) 잠시 침묵합니다...]`,
          action: 'insist',
          target: null,
        };
      }
